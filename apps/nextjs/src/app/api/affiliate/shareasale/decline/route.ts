  ...
  try {
    ...
    if (org?.shareASaleToken && org?.shareASaleSecretKey) {
      try {
        ...
        if (responseText.includes("Affiliate Declined")) {
          return NextResponse.json({ 
            success: true, 
        ...
        } else if (responseText.includes("Affiliate Already Declined")) {
          return NextResponse.json(
            { error: "Affiliate already declined" },
            { status: 400 }
          );
        } else {
          return NextResponse.json(
            { error: "Failed to decline affiliate" },
            { status: 500 }
          );
        }
      } catch (apiError) {
    ...
    const mockResponse = {
      success: true,
      message: `Affiliate ${affiliateId} declined successfully (Demo Mode)`,
      affiliateId
    };
